...
This project was a collaborative effort, with each team member contributing significantly to its successful completion. Our distinct roles and responsibilities ensured a comprehensive approach to both the technical implementation and the documentation of the Convolutional Neural Network (CNN) project.
...
- **Yeming Dai:** Focused on the core computational and structural components of the CNN. Dai Yeming was responsible for implementing the `Conv` (Convolutional Layer) class, the overarching `CNN` class that orchestrates the network, and the `main.cpp` file which serves as the program's entry point and handles model assembly and execution.
